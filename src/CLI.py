...
class CLI:
    ...
    def check_args(args: argparse.Namespace):
        if (args.targets <= 0):
            raise argparse.ArgumentTypeError(f"Argument 'target' must be greater than 0, given value: {args.target}")
    ...
    def run_pipeline(self, targets_count: int) -> bool:
        ...
        try:
            ...
            targets = [self.manager.run_container(f"target-{i}", self.NETWORK_NAME, network = network) for i in range(targets_count)]
